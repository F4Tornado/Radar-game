...
function generateTerrain() {
  ...
  for (let i = 0; i < width; i++) {
    for (let j = 0; j < height; j++) {
      let biomeValue = 256 - (Math.abs(noise.perlin2(j / 1000, i / 1000)) * 256)
      ...
      if (biomeValue < 160) { // Oceans
        ...
        heightmap.push(map(lerp(map(hillsMap[j * height + i], 0, 1, 0, 1), (1 - Math.abs(noise.simplex2(i / 200, j / 200))) ** 2, map(biomeValue, 190, 256, 0, 1) ** 2.5), 0, 1, 0.1, map(biomeValue, 190, 256, 0.1, 1) ** 0.7));
      }
    }
  }
